...
import com.linekong.game.activity.utils.DataFormatUtils.DateUtil;
...
import com.linekong.game.activity.utils.http.HttpUtils;
...
public class SendShortMessage {
    ...
    public static int sendMessage(String phoneNum,String phoneCode) throws UnsupportedEncodingException {
        String url = GuavaUtil.getConfigValue(SysCodeConstant.SEND_MESSAGE_URL);
        String password = GuavaUtil.getConfigValue(SysCodeConstant.SEND_MESSAGE_PASSWORD); //用户密码
        String send = DateUtil.getTimeString(new Date());
        String account = GuavaUtil.getConfigValue(SysCodeConstant.SEND_MESSAGE_ACOUNT);
        String sign = GuavaUtil.getConfigValue(SysCodeConstant.SEND_MESSAGE_SIGN_KEY);
        String content = GuavaUtil.getConfigValue(SysCodeConstant.SEND_MESSAGE_CONTENT);
        ...
        if (sendResult.contains(SysCodeConstant.DEL_SEND_MESSAGE_SUCCESS)) {
